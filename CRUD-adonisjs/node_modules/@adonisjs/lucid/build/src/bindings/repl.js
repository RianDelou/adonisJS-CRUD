...
export function defineReplBindings(app, Repl) {
     ...
     */
    Repl.addMethod('loadModels', async (repl) => {
        const modelsPath = app.modelsPath();
        console.log(repl.colors.dim(`recursively reading models from "${app.relativePath(modelsPath)}"`));
     ...
     */
    Repl.addMethod('loadDb', async (repl) => {
     ...
     */
    Repl.addMethod('loadFactories', async (repl) => {
        const factoriesPath = app.factoriesPath();
        console.log(repl.colors.dim(`recursively reading models from "${app.relativePath(factoriesPath)}"`));
        const { fsImportAll } = await import('@poppinss/utils');
